...
const Comments = ({postID}: any): JSX.Element => {
    ...
    useEffect(() => {
        ...
        axios.get(`https://blog-api-t6u0.onrender.com/posts/${postID}?_embed=comments`).then((res) => {
    ...
    const commentPost = async ():Promise<void> => {
        ...
        await axios.post(`https://blog-api-t6u0.onrender.com/comments`, {...comment,postId:postID}).then((res) =>{
    ...
    return (
        <Box>
            <Button variant="contained" color='primary' sx={{zIndex:'2',position:'sticky',top:'15vh',left:'85vw'}} onClick={() => setDialog(true)}>
                <AddComment/>
            </Button>
            {dialog &&
            <PostActionDialog
